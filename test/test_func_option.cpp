...
{
    bool catched = false;
    fo->enable_option({Option::OP_HW_NODE_CORE_HPTHREAD, Option::OP_VM_MEM_SAMPLE});
    try {
        fo->check_option(Func::FC_HW_NODE_NUM_P_);
    } catch (OpNotEnable e) {
...
{
    bool catched = false;
    fo->enable_option({Option::OP_VM_MISS_RATE, Option::OP_HW_NODE_CORE_HPTHREAD});
    fo->disable_option({Option::OP_HW_NODE_CORE_HPTHREAD});
    try {
        fo->check_option(Func::FC_HW_NODE_NUM_P_);
    } catch (OpNotEnable e) {
        cout << "catch exception OpNotEnable" << endl
